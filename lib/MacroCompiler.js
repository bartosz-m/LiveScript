...
var util, fs, Macro, Ast, runtimePatch, check, ASTCompiler, TypeSelector, AndSelector, SeriesSelector, nodeSelector, x$, macroExpansionSelector, y$, z$, z1$, z2$, z3$, macroDefinitionSelector, z4$, z5$, z6$, z7$, z8$, toMacroArgument, MacroCompiler, this$ = this;
util = require('util');
fs = require('fs');
Macro = require('./Macro');
Ast = require('./ast');
...
toMacroArgument = function(it){
  var result, i$, ref$, len$, item;
  if (it.value == null) {
    console.log(it);
  }
  if (it.items != null) {
    result = {};
    for (i$ = 0, len$ = (ref$ = it.items).length; i$ < len$; ++i$) {
      item = ref$[i$];
      result[item.key];
