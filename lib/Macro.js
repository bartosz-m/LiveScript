...
Macro = (function(){
  ...
  function Macro(name, ast, parent){
    ...
    if (!this.ast) {
      throw Error("Argument 'ast' nie może być " + toString$.call(this.ast).slice(8, -1));
    }
    this.type = 'Macro';
    matchBlockAst = matchBlockExpr.match(this.ast);
    if (matchBlockAst) {
      console.log("Mam matcher dla makra " + this.name);
      matcher = evaluateAst(matchBlockAst.args[0]);
      this.matcher = nodeSelector(matcher);
      matchBlockAst.block.removeChild(matchBlockAst.doWyjebania);
    }
